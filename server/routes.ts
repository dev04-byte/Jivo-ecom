...
// Configure multer for PO attachments with disk storage
const poAttachmentUpload = multer({
  storage: multer.diskStorage({
    destination: (req, file, cb) => {
      const uploadPath = "attachments/po";
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowedExtensions = ['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.csv', '.png', '.jpg', '.jpeg', '.txt', '.zip'];
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Setup PO with attachment routes
  setupPoWithAttachmentRoutes(app);
  ...
  // Modified PO endpoint to require attachment
  app.post("/api/pos", 
    // First middleware: validate multipart request
    (req, res, next) => {
      console.log("🚀🔥 NEW CODE IS ACTIVE - LATEST VERSION 2025-09-01 🔥🚀");
      ...
      if (!contentType.includes('multipart/form-data')) {
        console.log("❌ Non-multipart request - attachment required");
        return res.status(400).json({ 
          message: "Attachment is required. Please use multipart/form-data and include an attachment file.",
          error: "ATTACHMENT_REQUIRED",
          hint: "Use FormData and include a file with key 'attachment'"
        });
      }
      console.log("✅ Request is multipart/form-data - calling next() to proceed to multer");
      next();
    },
    // Second middleware: multer file processing
    poAttachmentUpload.single('attachment'),
    // Final route handler
    async (req, res) => {
    console.log("🚀 HIT THE PO ROUTE AFTER MULTER PROCESSING!");
    console.log("Method:", req.method);
    console.log("URL:", req.url);
    ...
    // Debug: Check all possible places where data might be
    console.log("🔍 ALL REQUEST PROPERTIES:");
    console.log("- req.query:", req.query);
    console.log("- req.params:", req.params);
    console.log("- req.body type:", typeof req.body);
    console.log("- req.body constructor:", req.body?.constructor?.name);
    
    // Check if poData exists as a string field
    if (req.body && req.body.poData) {
      console.log("✅ Found poData field:", req.body.poData);
    } else {
      console.log("❌ poData field missing from req.body");
      console.log("Available body fields:", Object.keys(req.body || {}));
    }
    
    try {
      // Double-check if attachment is provided (after multer processing)
      if (!req.file) {
        console.log("❌ Missing attachment file after multer processing");
        return res.status(400).json({ 
          message: "Attachment is required. Please upload a file with your PO submission.",
          error: "ATTACHMENT_REQUIRED"
        });
      }
      ...
      // Parse PO data from request body
      let poData;
      try {
        console.log("📝 Raw body:", req.body);
        console.log("📝 Body.poData:", req.body.poData);
        ...
        if (req.body.poData) {
          ...
          // Try to use the body directly
          poData = req.body;
        }
        
        console.log("📝 Parsed poData:", poData);
      } catch (parseError) {
        console.log("❌ Failed to parse PO data:", parseError);
        return res.status(400).json({ 
          message: "Invalid PO data format. Please send valid JSON.",
          error: "INVALID_JSON"
        });
      }
      ...
      if (!poData.master) {
        ...
        console.log("Available fields:", Object.keys(poData));
        return res.status(400).json({ 
      ...
      console.log("✅ Request has correct structure with attachment");
      
      // Create the PO first
      const { master, lines } = poData;
      ...
      const savedAttachment = await storage.addPoAttachment(attachmentData);

      console.log("✅ PO created successfully with attachment");
      ...
      res.status(500).json({ message: "Failed to create purchase order with attachment", error: error instanceof Error ? error.message : String(error) });
    }
  });
