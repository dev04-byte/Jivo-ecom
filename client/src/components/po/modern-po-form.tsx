...
export function ModernPOForm({ onSuccess, editMode = false, editPoId }: ModernPOFormProps = {}) {
  ...
  const createPOMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      try {
        if (editMode && editPoId) {
          // Update existing PO - use JSON for updates
          const { attachedFile, ...updateData } = data;
          const response = await apiRequest('PUT', `/api/pos/${editPoId}`, updateData);
          return response;
        } else {
          // Create new PO - use multipart/form-data for attachments
          const formData = new FormData();
          
          // Add the attachment file
          if (data.attachedFile) {
            formData.append('attachment', data.attachedFile);
          }
  ...
  const onSubmit = useCallback((data: POFormData) => {
    ...
    if (!attachedFile) {
      toast({
        ...
        description: "Please upload an attachment before submitting the Purchase Order.",
        variant: "destructive",
      });
      return;
    }
