...
export interface IStorage {
  ...
  // PO Attachment methods
  addPoAttachment(attachmentData: any): Promise<any>;
  getPoAttachments(poId: number, poType: string): Promise<any[]>;
  getPoAttachmentById(id: number): Promise<any | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // PO Attachment methods
  async addPoAttachment(attachmentData: any): Promise<any> {
    const [attachment] = await db
      .insert(poAttachments)
      .values(attachmentData)
      .returning();
    return attachment;
  }

  async getPoAttachments(poId: number, poType: string): Promise<any[]> {
    return await db
      .select()
      .from(poAttachments)
      .where(and(eq(poAttachments.po_id, poId), eq(poAttachments.po_type, poType), eq(poAttachments.is_active, true)));
  }

  async getPoAttachmentById(id: number): Promise<any | undefined> {
    const [attachment] = await db
      .select()
      .from(poAttachments)
      .where(and(eq(poAttachments.id, id), eq(poAttachments.is_active, true)));
    return attachment;
  }
  ...
  // Distributor methods
  async getAllDistributors(): Promise<Distributors[]> {
  ...
  async createPoInExistingTables(masterData: any, linesData: any[]): Promise<any> {
    ...
    return await db.transaction(async (tx) => {
      // Find distributor ID by name if serving_distributor is provided, otherwise use provided distributor_id
      let distributorId = masterData.distributor_id || 1; // Use provided distributor_id or default to 1
      if (masterData.serving_distributor) {
        try {
          const distributor = await tx
      ...
      const insertValues = {
        ...
        vendor_po_number: masterData.vendor_po_number || masterData.po_number,
