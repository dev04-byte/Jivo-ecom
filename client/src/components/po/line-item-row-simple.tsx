...
export function LineItemRowSimple({ item, platformId = 0, onUpdate, onRemove, index = 0 }: LineItemRowProps) {
  ...
  const { data: hanaItemsResponse, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch from HANA via stored procedure
      const result = await sqlService.searchHanaItems({ 
        search: debouncedSearchTerm, 
        limit: 50,
        platformId: platformId 
      });
      
      console.log('Search result from HANA:', result);
      return result;
    },
  ...
  }, [item.basic_rate, item.gst_rate, item.quantity]);
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-xs font-medium text-blue-700">
                          {platformId ? `Platform-specific catalog (ID: ${platformId})` : "All items catalog"}
                        </span>
                        {hanaItemsResponse?.metadata?.searchTerm && (
                          <span className="text-xs text-blue-600">
                            Searching for: "{hanaItemsResponse.metadata.searchTerm}"
                          </span>
                        )}
                      </div>
                      {hanaItemsResponse?.metadata?.source && (
                        <Badge variant="outline" className="text-xs bg-green-100 text-green-700">
                          {hanaItemsResponse.metadata.source}
                        </Badge>
                      )}
                    </div>
                    {hanaItemsResponse?.metadata?.filteredRecords !== undefined && (
                      <div className="mt-2 text-xs text-blue-600">
                        Found {hanaItemsResponse.metadata.filteredRecords} items
                        {hanaItemsResponse.metadata.totalRecords && hanaItemsResponse.metadata.totalRecords !== hanaItemsResponse.metadata.filteredRecords && (
                          <span> (filtered from {hanaItemsResponse.metadata.totalRecords} total)</span>
                        )}
                      </div>
                    )}
                  </div>
